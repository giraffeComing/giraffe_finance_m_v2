<template>
  <div>
    <div class="record">
      <div class="item" v-for="(item , index) in fundData.record">
        <router-link :to="{ name: 'VFunHistory'}">
          <div class="title">
            <span class="bar red">申</span>
            <span class="name">{{item.fundname}}（{{item.fundcode}}）</span>
            <span class="lab">确认中</span>
          </div>
          <div class="info">
      ...
      <!--暂无记录-->
      <v-empty v-if="fundData.record.length<1" :opts="emptyData"></v-empty>
    </div>
  </div>
</template>
